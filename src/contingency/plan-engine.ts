...
function seedSectionsToPlanSections(seedSections: SeedSection[]): PlanSection[] {
...
export function createPlan(template: PlanTemplate, municipalityData: MunicipalityData): Plan {
  const ts = nowIso();
...
export function generateFromSeed(template: PlanTemplate, seedContent: SeedBundle): Plan {
...
export function addSection(plan: Plan, section: PlanSection): Plan {
...
export function updateSection(planId: string, sectionKey: string, content: PlanSection['content_blocks']): Plan {
  const stored = planStore.get(planId);
  if (!stored) throw new Error(`Plan not found: ${planId}`);
...
export function setPlan(plan: Plan): Plan {
