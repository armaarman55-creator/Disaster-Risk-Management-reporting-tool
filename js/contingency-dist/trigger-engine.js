...
export function createTriggerLevel(level) {
    if (!level.level_code?.trim())
        throw new Error('Trigger level_code is required');
    if (!level.level_name?.trim())
        throw new Error('Trigger level_name is required');
    if (!Number.isFinite(level.severity_rank))
        throw new Error('Trigger severity_rank must be numeric');
    return {
        id: level.id ?? uid('trg'),
...
export function attachTriggerToPlan(planId, trigger) {
