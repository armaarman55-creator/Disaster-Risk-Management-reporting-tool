...
function nextVersionLabel(currentHistory) {
...
function buildSnapshot(plan, actorUserId) {
    return {
        id: uid('vsn'),
        plan_id: plan.id,
...
export function saveVersionSnapshot(plan, actorUserId) {
...
function addApprovalEvent(plan, fromStatus, toStatus, actorUserId, note) {
    const event = {
        id: uid('apr'),
        plan_id: plan.id,
...
export function updatePlanStatus(planId, status, actorUserId, note) {
...
export function submitForReview(planId, actorUserId, note) {
    return updatePlanStatus(planId, 'in_review', actorUserId, note ?? 'Submitted for review');
}
export function approvePlan(planId, actorUserId, note) {
