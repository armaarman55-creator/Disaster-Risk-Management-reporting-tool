...
function renderTabContent(data: AssistantResponse): void {
  ...
  let html = '';
  if      (_currentTab === 'legislation')  html = renderLegislation(data);
  else if (_currentTab === 'preview')      html = renderGuide(data);
  else if (_currentTab === 'suggestions')  html = renderSuggestions(data);
  else if (_currentTab === 'live')         html = renderLive(data);
  else if (_currentTab === 'layout')       html = renderMockLayout(data);
  else                                      html = renderLegislation(data);
...
// ─── Tab: Plan Layout ─────────────────────────────────────────────────────────
function renderMockLayout(data: AssistantResponse): string {
  ...
  const bs = (color: string) =>
    `display:inline-block;font-size:9.5px;font-weight:700;padding:1px 7px;border-radius:999px;color:#fff;background:${esc(color)};margin-left:6px;vertical-align:middle`;
  ...
  layout.sections.forEach((section: any, i: number) => {
    ...
    const bi: Record<string, string> = { text: '¶', table: '⊞', list: '≡' };
    const blockColors: Record<string, { bg: string; color: string; border: string }> = {
